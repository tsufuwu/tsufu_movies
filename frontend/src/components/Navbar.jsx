...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const data = await searchMovies(query.trim(), 1, honeypot)
  ...
  const SuggestionDropdown = ({ onSearchAll, isMobile = false }) => (
    <div className={`${isMobile ? 'relative mt-2' : 'absolute top-full right-0 mt-3'} w-full bg-[#181818] border border-gray-800 rounded-xl shadow-[0_15px_40px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col z-50`}>
      {loadingSuggestions ? (
        ...
        <div>
          ...
          {/* "Tìm kiếm với từ khóa" row */}
          <div
            onClick={onSearchAll}
            className="p-3 text-center text-sm font-bold text-[#E50914] hover:bg-white/5 cursor-pointer transition-colors border-t border-gray-800"
          >
            🔍 Tìm kiếm với từ khóa &ldquo;{query}&rdquo;
          </div>
        </div>
      ) : (
        <div>
          ...
          {/* Still show "Tìm kiếm với từ khóa" even when no results */}
          <div
            onClick={onSearchAll}
            className="p-3 text-center text-sm font-bold text-[#E50914] hover:bg-white/5 cursor-pointer transition-colors border-t border-gray-800"
          >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            ...
            <form onSubmit={handleSearch} className="hidden sm:flex items-center relative" ref={searchRef}>
              ...
              {/* Gợi ý tìm kiếm Desktop (Dropdown) */}
              {showSuggestions && (query.trim().length > 0) && (
                <div className="absolute top-full right-0 mt-3 w-80 z-50">
                  <SuggestionDropdown onSearchAll={handleSearch} />
                </div>
              )}
            </form>
        ...
        {menuOpen && (
          <div className="md:hidden bg-[#141414] border border-gray-800 rounded-2xl mt-4 p-5 mb-4 shadow-2xl" style={fontStyle}>
            <form onSubmit={handleSearch} className="mb-4" ref={mobileSearchRef}>
              {/* Anti-bot Honeypot input */}
              <input
                type="text"
                name="website"
              ...
              <div className="relative">
                <input
                  ...
                  enterKeyHint="search"
                  value={query}
                  onChange={(e) => { setQuery(e.target.value); setMobileShowSuggestions(true); }}
                  onFocus={() => { if(query) setMobileShowSuggestions(true); }}
                  placeholder="Tìm kiếm phim..."
                  className="w-full bg-black/50 border border-gray-800 rounded-xl px-5 py-3 text-base text-white placeholder-gray-500 focus:outline-none focus:border-[#E50914] pr-12"
                />
                <button
                  type="submit"
              ...
              {mobileShowSuggestions && (query.trim().length > 0) && (
                <SuggestionDropdown onSearchAll={handleSearch} isMobile={true} />
              )}
            </form>
